...
int main(void) {
	...
	FILE* fp1 = fopen("wm_g100_beneficial.txt", "w");
	FILE* fp2 = fopen("wm_g100_neutral.txt", "w");
	FILE* fp3 = fopen("wm_g100_N.txt", "w");
	...
	for (int i = 0; i < rounds; ++i) {
		...
		for (int j = 1; j < num_gen; ++j) {
			...
			double* wi = (double *) malloc(sizeof(double) * num_ind * 10);
			double wsum = 0;
			for (int i = 0; i < N; ++i) {
				wi[i] = pow(1+s, ori_bn[i]);
				wsum += wi[i];
			}

			double wmean = wsum / (double) N;

			for (int i = N; i < num_ind * 10; ++i) {
				wi[i] = 0;
			}
			...
			int idx = 0;
			for (int k = 0; k < N; ++k) {
				double lambda = (double) ((1 + g) * pow(1+s, ori_bn[k])) / (double) (wmean * (1 + g * N / K));
